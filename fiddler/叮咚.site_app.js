						...
						value: function() {
							...
							if (!this._isInitializeComplete) {
								...
								b.scale.set(i), b.y = 760 - o, b.on("reset", (function(e) {})), b.on("start", (function(e) {})), b.on("end", function() {
											...
											.mark((function t(i) {
												return h()
													.wrap((function(t) {
														for (;;) switch (t.prev = t.next) {
															case 0:
																e.gameTime += b.gameTime, e.freeResNum > 0 ? (e.freeResNum -= 1, e.gameResurrection(), e.view.life.gotoAndStop(2 - e.freeResNum)) : e.shareResNum > 0 ? (e.view.life.gotoAndStop(3), A.show({
										...
										//alert(t.addScore)
										/*C.push(J[t.addScore]),*/ console.log("_gameScoreList", C), e.myScore.text = b.score.toString()
											.padStart(6, "0")
										...
										try{
											...
											//first=1000
											 var oo = {
												...
												var C=[];q=[];
												var ss=[100,250,350,500]
												for(var score=0;score<first+500;){
													var ran=Math.random();
													var index=0;
													if(ran>0.95) index=3;
													else if(ran>0.8) index=2;
													else if(ran>0.5) index=1;
													else if(ran<=0.5) index=0;
													C.push(J[ss[index]])
													score=score+ss[index];
													q.push(parseInt(9*Math.random())+1)
													q.push(parseInt(9*Math.random())+1)
													q.push(parseInt(9*Math.random())+1)
													q.push(parseInt(9*Math.random())+1)
														X=X+7;
												...
												var len=C.length;
												var first=parseInt(len/3)+3;
												var sec=2*parseInt(len/3)+3;
												var tmp1=C[first];
												var tmp2=C[sec];
												C[first]="e";
												C[sec]="e";
											 ...
											 // alert(score)
												 ...
												 // return;
												//b.score="400";
												this.gameTime=(Date.parse(new Date) - this.startTime)-2331-parseInt(1000*Math.random())*/
